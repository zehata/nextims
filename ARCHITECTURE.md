...
Performance is not going to be an issue considering that compute will likely be minimal, operations will be I/O bound, and we will have load balancers to scale up the deployment when required.
# Architecture
## Frontend
### Framework
...
- Having tried using SSR before on my portfolio site, there are some "gotchas" when it comes to hydration. While it is not difficult to solve, this will be a point of consideration when writing any client-side component, for no significant benefits in return.
...
- It will be prudent not to try anything too flashy and distracting. (Definitely not going to be designed with the same style as my portfolio page)
...
## Backend
| Candidates        | Technical                                                                                       | Ergonomics                                                                                                                                | Language                                                    | Ecosystem for Postgres                                        | OpenAPI support | Other remarks                                                                                                                                      |
|-------------------|-------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------|---------------------------------------------------------------|-----------------|----------------------------------------------------------------------------------------------------------------------------------------------------|
| FastAPI (Python)  | Performant, well-used (15.1%)                                                                   | Simple                                                                                                                                    | Python                                                      | Psycopg + Jinja for templating                                | Yes             |                                                                                                                                                    |
| Fastify (Node.js) | More performant than Express, more batteries included, well-used but less commonly known (3.1%) | Developers' sentiment seems to prefer Fastify because it's more modern and more performant, albeit, from what I have read, marginally so. | JavaScript by default.<br>TypeScript can optionally be used | Slonik for SQL templating and connecting to the sql instances | Yes             |                                                                                                                                                    |
| Express (Node.js) | Sufficiently performant, well-used (20.3%, highest framework on SO dev survey).                 | Simple                                                                                                                                    | JavaScript by default. TypeScript can optionally be used    | Same as above                                                 | Through swagger |                                                                                                                                                    |
| NestJS (Node.js)  | Based off of Express. Comparable to Express. Optionally can use Fastify.                        | Fully-featured                                                                                                                            | TypeScript                                                  | Same as above                                                 | Yes             | This is going to be a bit silly, but NestJS is going to sound confusing in all sorts of documents and conversations given the similarity to NextJS |
...
~~I have heard about the OpenAPI standard in online forums and someone mentioned to me that I should look it up.~~ OpenAPI documentations are automatically generated by some frameworks such as FastAPI, and other frameworks offer API generation via a plugin.

I have picked FastAPI because:
- I believe it is the framework that is currently being used by the team
- It is modern, well-used and stable
- Automatic user documentation and OpenAPI documentation generation allows us to focus on improving other aspects of the system.
...
- Statically generated documentation site, with ability to pull in API definitions from the backend codebase to generate code samples
	...
	- Because the API documentation is expected to be mostly static, there is no need for us to be using a dynamic site. Definitely no backend for this